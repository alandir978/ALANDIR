...
export function ItalianStocks() {
  ...
  return (
    <article className="flex flex-col rounded-2xl border border-border bg-card p-5">
      ...
      <div className="mt-4 grid grid-cols-[1.7fr_1fr_0.9fr_0.7fr] gap-2 border-b border-border pb-2 text-[10px] font-bold tracking-wide text-muted-foreground">
        <span>TITOLO</span>
      ...
      <ul className="flex-1 divide-y divide-border/60">
        {market.stocks.map((stock) => (
          <li
            key={stock.ticker}
            className="grid grid-cols-[1.7fr_1fr_0.9fr_0.7fr] items-center gap-2 py-2.5 text-sm transition-colors hover:bg-panel/40"
          >
            <span className="flex items-center gap-2.5 font-medium">
              <span className="grid size-7 shrink-0 place-items-center rounded-md border border-border bg-panel text-[9px] font-bold text-gold/90">
                {stock.ticker.slice(0, 2)}
              </span>
              <span className="flex min-w-0 flex-col leading-tight">
                <span className="text-[11px] font-bold tracking-wide text-foreground/95">
                  {stock.ticker}
                </span>
      ...
      >
        Vedi tutti i titoli {market.label}
